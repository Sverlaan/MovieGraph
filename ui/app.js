...
// ═══════════════════════════════════════════════════════════
const API_URL         = "/ask";
const POSTER_FALLBACK = "https://via.placeholder.com/140x210?text=No+Image";
...
const cypherSection  = document.getElementById("cypher-section");
...
async function loadExamples() {
  try {
    ...
    questions.forEach(q => {
      ...
      chip.textContent = q;
      chip.addEventListener("click", () => { inputEl.value = q; inputEl.focus(); });
...
function setLoading(on) {
  ...
  askBtn.disabled = on;
  if (on) { answerEl.textContent = ""; answerEl.classList.remove("styled"); hideAll(); }
}
...
function hideAll() {
  ...
  cypherSection.style.display = "none";
...
async function renderResponse(data) {
  ...
  if (cypher) {
    cypherCode.innerHTML = highlightCypher(escHtml(cypher));
    cypherSection.style.display = "block";
  }
}
...
// CARD RENDERERS
// ═══════════════════════════════════════════════════════════
const MOVIE_PRIMARY  = new Set(["title","year","poster","slug"]);
...
function renderMovieGrid(results) {
  ...
  results.forEach(movie => {
    ...
    card.className = "movie-card card-animate";
    card.innerHTML = `
      <img class="poster" src="${movie.poster || POSTER_FALLBACK}"
           alt="${escHtml(movie.title || "")}" onerror="this.src='${POSTER_FALLBACK}'" />
      <div class="movie-title">${escHtml(movie.title || "—")}${movie.year ? ` <span style="font-weight:400">(${movie.year})</span>` : ""}</div>
      ${extraFields(movie, MOVIE_PRIMARY).length
        ? `<div class="movie-meta">${extraFields(movie, MOVIE_PRIMARY).map(formatField).join(" · ")}</div>`
        : ""}`;
...
function renderPersonGrid(results) {
  ...
  results.forEach(person => {
    ...
    const card = document.createElement("div");
    card.className = "person-card card-animate";
    card.innerHTML = `
      ...
      ${extraFields(person, PERSON_PRIMARY).length
        ? `<div class="person-meta">${extraFields(person, PERSON_PRIMARY).map(formatField).join(" · ")}</div>`
        : ""}`;
...
function renderPersonDetail(person) {
  ...
  personDetailEl.innerHTML = `
    ...
    <div class="detail-body">
      ...
      ${extra.length ? `<div class="detail-meta">${extra.map(formatField).join(" · ")}</div>` : ""}
      ${person.biography ? `<div class="detail-plot">${escHtml(person.biography)}</div>` : ""}
      ${links.length    ? `<div style="font-size:.82rem;margin-top:.5rem">${links.join(" · ")}</div>` : ""}
    </div>`;
}

const TAG_TYPE_CLASS = {
  genres:      "genre",
  mini_themes: "minitheme",  // snake_case (dedicated movie endpoint)
  miniThemes:  "minitheme",  // camelCase (LLM-generated Cypher)
  themes:      "theme",
  studios:     "studio",
  countries:   "country",
  languages:   "language",
  collection:  "collection",   // singular (dedicated endpoint)
  collections: "collection",   // plural (LLM-generated Cypher)
  keywords:    "keyword",
  oscar_noms:  "oscarnom",
};
...
function renderMovieDetail(movie) {
  ...
  movieDetailEl.innerHTML = `
    ...
    <div class="detail-content">
      <img class="detail-poster" src="${movie.poster || POSTER_FALLBACK}"
           alt="${escHtml(movie.title || "")}" onerror="this.src='${POSTER_FALLBACK}'" />
      <div class="detail-body">
        <h2>${escHtml(movie.title || "—")}${escHtml(year)}</h2>
        ${metaParts.length ? `<div class="detail-meta">${metaParts.join(" · ")}</div>` : ""}
        ${movie.tagline ? `<div style="font-style:italic;color:#555;font-size:.88rem">${escHtml(movie.tagline)}</div>` : ""}
        ${movie.plot    ? `<div class="detail-plot">${escHtml(movie.plot)}</div>` : ""}
        ${links.length  ? `<div style="font-size:.82rem;margin-top:.5rem">${links.join(" · ")}</div>` : ""}
      </div>
    </div>
    ${tags.length ? `<div class="detail-tags">${tags.map(({text, cls}) => `<span class="detail-tag${cls ? ` detail-tag--${cls}` : ''}">${escHtml(text)}</span>`).join("")}</div>` : ""}`;
}

// ═══════════════════════════════════════════════════════════
// GRAPH RENDERER (D3 v7 force simulation)
// ═══════════════════════════════════════════════════════════
const LABEL_COLOR = {
  Movie:      "#f5c518",
  Person:     "#60a5fa",
...
function renderGraph(graphData) {
  ...
  if (!graphData || !graphData.nodes.length) {
    document.getElementById("graph-legend").innerHTML =
      `<span style="color:#aaa;font-size:.8rem">No graph data for this query</span>`;
    return;
  }
  ...
  node.append("circle")
    ...
    .attr("filter", "drop-shadow(0 1px 3px rgba(0,0,0,.18))");
  ...
  node.append("title").text(d => {
    ...
    const name = p.title || p.name || p.username || "?";
    return `[${(d.labels||[]).join(",")}] ${name}`;
  });
...
function formatField({ key, value }) {
  ...
  return `${label}: <strong>${escHtml(String(value))}</strong>`;
}
